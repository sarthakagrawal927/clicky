...
| `EnableActions` | `false` | `true` → action tags from the planner (`[CLICK:...]`, `[TYPE:...]`, `[OPEN_APP:...]`, `[VOLUME:...]`, `[BRIGHTNESS:...]`, etc.) are executed via local macOS APIs |
...
**Menu Bar Panel Pattern**: `PaceMenuBarOverlayManager` owns the visible black menu-bar/notch capsule in a top-level non-activating `NSPanel`. `MenuBarPanelManager` owns the floating companion panel plus a tiny fallback `NSStatusItem` anchor. This gives full control over appearance and avoids the standard macOS menu/popover chrome. The panel is non-activating so it doesn't steal focus. A global event monitor auto-dismisses it on outside clicks.
...
| `MenuBarPanelManager.swift` | ~303 | Fallback NSStatusItem + custom NSPanel lifecycle. Manages the floating companion panel (show/hide/position), can anchor it to the menu-bar overlay frame, and installs click-outside-to-dismiss monitor. |
...
| `CompanionPanelView.swift` | ~587 | SwiftUI panel content for the menu bar dropdown. Shows companion status, push-to-talk instructions, local model status, read-screen toggle, permissions UI, and quit button. Dark aesthetic using `DS` design system. |
