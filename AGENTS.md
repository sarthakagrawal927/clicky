...
| `CompanionPanelView.swift` | ~1068 | SwiftUI panel content for the menu bar dropdown. Shows companion status, push-to-talk instructions, local model status, TTS voice quality, read-screen toggle, cursor annotation toggle, action approval toggle, watch-mode toggle, core permission rows, local-tool permission preflight rows, recent action results, local memory summary, settings-window entry point, and quit button. Dark aesthetic using `DS` design system. |
| `PaceSettingsWindow.swift` | ~534 | Normal macOS settings window for management surfaces too large for the notch panel. Provides tabs for General preferences, MCP server config, permissions, voice, local memory, and recent action history. Uses the same `CompanionManager` state and preference setters as the panel. |
...
| `PaceToolRegistry.swift` | ~333 | Typed local tool catalog. Defines canonical names, aliases, schema examples, descriptions, risk levels, execution summaries, observation summaries, and planner prompt generation. Notes supports create/append/search actions through the same registry entry. MCP actions use a generic external-tool risk label because their exact semantics live in the configured server. |
| `PaceActionExecutor.swift` | ~2087 | Local action execution layer with screenshot-pixel → CG-global coordinate conversion (mirrors the pointing logic in CompanionManager). Single-clicks try `PaceAXTargeter` first; falls back to CGEvent. Also defines `PaceActionTagParser`, grouped `PaceActionExecutionPlan`, approval summaries, and tool-observation types that feed planner turns plus fallback user feedback. Parses `<tool_calls>` JSON plus legacy tags for clicks, typing, keys, scroll, app/URL opening, Music, volume, brightness, Calendar reads, Reminder creation, Finder, Notes create/append/search, Mail drafts, Things, Shortcuts, Messages opening, and generic MCP tool calls. URL opening honors local memory's preferred browser. Action execution is gated by Info.plist `EnableActions`. |
