...
- **Watch mode**: `PaceScreenWatchModeController` is the explicit watch-loop primitive. It samples screenshots, uses `PaceScreenImageDiffer`, and emits events only when a screen has meaningful visual change. The companion panel exposes a `Watch Mode` toggle; optional voice start/stop is still pending.
...
| `PaceScreenWatchMode.swift` | ~142 | Explicit watch-mode primitive. Samples all screens at an interval, compares fingerprints, throttles repeated changes, and emits meaningful-change events for future panel/voice watch mode. |
